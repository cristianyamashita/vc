(() => {
  ...
  async function seedScenarios() {
    ...
    for (const item of existing) {
      if (item.id === 'hotel' || (item.builtIn && !presetIds.has(item.id))) {
        await idb('scenarios', 'readwrite', store => store.delete(item.id));
        await idb('saves', 'readwrite', store => store.delete(item.id));
      }
    }
    const remaining = existing.filter(item => item.id !== 'hotel' && !(item.builtIn && !presetIds.has(item.id)));
    for (const preset of PRESETS) {
      const current = remaining.find(item => item.id === preset.id);
